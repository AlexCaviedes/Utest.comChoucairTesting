...
public class UtestStepDefinitions {
    ...
    public void luegoContinuaConElRegistroDeSusDatosEnLosFormularios(List<UtestData>utestData) {
        OnStage.theActorInTheSpotlight().wasAbleTo(Register.OnThePage(utestData.get(0).getFirst_Name(), utestData.get(0).getLast_Name(), utestData.get(0).getEmail_Address(), utestData.get(0).getMonth_Birth(), utestData.get(0).getDay_Birth(), utestData.get(0).getYear_Birth(), utestData.get(0).getCity_City(), utestData.get(0).getPostal_Code(), utestData.get(0).getCreate_Password(), utestData.get(0).getConfirm_Password()));
    }

    @Then("^Por ultimo da click y lee el mensaje del boton (.*)$")
    public void porUltimoDaClickYLeeElMensajeDelBotonCompleteSetup(String question) {
